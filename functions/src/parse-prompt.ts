export const PARSE_PROMPT = (today: string) => `
  ...
  - departureLocation: Where the user is departing from. Be careful to distinguish this from the destination.
  - destination: Where the user wants to go. Do not confuse this with the departure location.
  - startDate: The start date in YYYY-MM-DD format. If a season or month is mentioned, pick a reasonable date. Today is ${today}.
  ...
  Be smart about dates. For example, if they say "next month", calculate it based on today's date (${today}).
